...
/**
 * [공사일지 저장소]
 * 데이터베이스의 'CONSTRUCTION_LOG' 테이블에 접근하여
 * 데이터의 저장, 조회, 수정, 삭제(CRUD)를 담당하는 인터페이스입니다.
 * Spring Data JPA가 실행 시점에 자동으로 구현체를 생성해줍니다.
 */
@Repository
public interface ConstructionLogRepository extends JpaRepository<ConstructionLog, Long> {
    ...
    /**
     * 1. [나의 일지 전체 조회]
     * 특정 작성자(author)가 작성한 모든 공사일지를 조회합니다.
     * - OrderByLogDateDesc: 날짜(logDate) 기준 내림차순(최신순)으로 정렬합니다.
     *
     * @param author 작성자 아이디 (로그인 사용자)
     * @return 해당 사용자의 공사일지 리스트 (최신순)
     */
     ...
     * @return 가장 최근 작성된 공사일지 (존재하지 않을 수 있으므로 Optional 반환)
     */
    Optional<ConstructionLog> findTopByAuthorOrderByLogDateDesc(String author);
}